...
int main(void) {
    ...
    for (int f = 0; f < mob->size; f++) {
        ...
        for (int i = 0; i < mob[f].fieldInstanceCount; i++) {
            ...
            if (strcmp(mob[f].fieldInstances_data_ptr[i].type, fieldInstance_array_point) == 0 ||
                strcmp(mob[f].fieldInstances_data_ptr[i].type, fieldInstance_point) == 0) {


                if (mob[f].fieldInstances_data_ptr[i].size > 0) {
                    printf(
                        "Field Instance points Size: %d\n",
                        mob[f].fieldInstances_data_ptr[i].fieldInstances_points_ptr->size);
                    for (int g = 0;
                         g < mob[f].fieldInstances_data_ptr[i].fieldInstances_points_ptr->size;
                         g++) {

                        // Cheat check to handle NULL values until I find a better way
                        if (mob[f].fieldInstances_data_ptr[i].fieldInstances_points_ptr[g].check !=
                            1)
                            break;
                        printf(
                            "Point X:%d,Y:%d\n",
                            mob[f].fieldInstances_data_ptr[i].fieldInstances_points_ptr[g].cx,
                            mob[f].fieldInstances_data_ptr[i].fieldInstances_points_ptr[g].cy);
                    }
                }
            ...
            if (strcmp(mob[f].fieldInstances_data_ptr[i].type, fieldInstance_array_int) == 0 ||
                ...
                if (mob[f].fieldInstances_data_ptr[i].size > 0) {
                    printf(
                        "Field Instance Ints Size: %d\n",
                        mob[f].fieldInstances_data_ptr[i].fieldInstances_ints_ptr->size);
                    for (int g = 0;
                         g < mob[f].fieldInstances_data_ptr[i].fieldInstances_ints_ptr->size;
                         g++) {

                        // Cheat check to handle NULL values until I find a better way
                        if (mob[f].fieldInstances_data_ptr[i].fieldInstances_ints_ptr[g].check != 1)
                            break;
                        printf(
                            "Int Value: %d\n",
            ...
            if (strcmp(mob[f].fieldInstances_data_ptr[i].type, fieldInstance_color) == 0 ||
                ...
                if (mob[f].fieldInstances_data_ptr[i].size > 0) {
                    ...
                    for (int g = 0;
                         g < mob[f].fieldInstances_data_ptr[i].fieldInstances_strings_ptr->size;
                         g++) {
                        if (mob[f].fieldInstances_data_ptr[i].fieldInstances_strings_ptr[g].check !=
                            1)
                            break;
                        if (mob[f]
                                .fieldInstances_data_ptr[i]
                                .fieldInstances_strings_ptr[g]
                                .strValue != NULL) {
                            printf(
                                "String: %s\n",
                                mob[f]
            ...
            if (strcmp(mob[f].fieldInstances_data_ptr[i].type, fieldInstance_float) == 0 ||
                ...
                if (mob[f].fieldInstances_data_ptr[i].size > 0) {
                    ...
                    for (int g = 0;
                         g < mob[f].fieldInstances_data_ptr[i].fieldInstances_floats_ptr->size;
                         g++) {
                        if (mob[f].fieldInstances_data_ptr[i].fieldInstances_floats_ptr[g].check !=
                            1)
                            break;
            ...
            if (strncmp(
                    mob[f].fieldInstances_data_ptr[i].type, fieldInstance_array_localEnum, 15) ==
                    0 ||
